...
# Last updated: <19/02/2020 16:05:07 (BrianM)>
...
def artefact_removal_examples():
    ...
    eeg_art = preprocessing_EEG.remove_artefacts(data_st['eeg_data'].copy(), data_st['ch_labels'], data_st['Fs'],
                                                 data_st['eeg_data_ref'], data_st['ch_labels_ref'])[0]
...
def preprocessing_and_feature_extraction():
    ...
    data_st['eeg_data'] = preprocessing_EEG.LPF_zero_phase(data_st['eeg_data'].copy(), data_st['Fs'])
    data_st['eeg_data'], data_st['Fs'] = preprocessing_EEG.signal_downsample(data_st['eeg_data'].copy(),
                                                                             data_st['Fs'], 64)
...
def load_edf_preprocessing_and_feature_extraction():
    ...
    utils.save_pd_as_edf(eeg_ref, 'demo_data.edf', Fs=Fs)  # Save the fake data as edf to simulate opening a real edf
    ...
    print('Loading "demo_data.edf", performing artefact removal, low pass filtering and down-sampling')
