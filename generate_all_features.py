...
# Last updated: <17/02/2020 16:45:05 (BrianM)>
...
def generate_all_features(data, save_p_l=None, channel_names=None, feat_set=None, params=None):
    ...
    feats_median_ch = pd.DataFrame(data=tmp2[:, :98], columns=feat_pd_names[:98])  # all names except IBI
    ...
    for n in range(N_feats):
        ...
        if feat_group in ['amplitude', 'spectral', 'rEEG', 'FD']:
            ...
            for c in range(N_channels):
                x_epochs = overlap_epochs(eeg_data[ch_labels[c]], Fs, params['EPOCH_LENGTH'], params['EPOCH_OVERLAP'])
            ...
            feats_median_ch = add_feat_data_to_array(feats_median_ch, np.median(feats_per_epochs, axis=0),
                                                     feat_pd_names, feat_set[n], params)
            feats_median_all = add_feat_data_to_array(feats_median_all, np.array([np.median(feats_channel, axis=0)]),
                                                      feat_pd_names, feat_set[n], params)
        # ---------------------------------------------------------------------
        ...
        # ---------------------------------------------------------------------
            ...
            for c in range(N_channels):
                tmp = overlap_epochs(eeg_data[ch_labels[c]], Fs, params['EPOCH_LENGTH'], params['EPOCH_OVERLAP'])
                if c == 0:
            ...
            feats_median_ch = add_feat_data_to_array(feats_median_ch, feats_epochs,
                                                     feat_pd_names, feat_set[n], params)
            feats_median_all = add_feat_data_to_array(feats_median_all, np.array([np.nanmedian(feats_epochs, axis=0)]),
                                                      feat_pd_names, feat_set[n], params)
            ...
            feats_channel.fill(np.nan)
            for c in range(N_channels):
                feats_channel[c, :] = IBI_features.main_IBI(eeg_data[ch_labels[c]].to_numpy(), Fs, feat_set[n])

            # and the median over all channels
            feat_st[feat_set[n]] = np.nanmedian(feats_channel, axis=0)
            feats_median_all = add_feat_data_to_array(feats_median_all, np.array([np.nanmedian(feats_channel, axis=0)]),
                                                      feat_pd_names, feat_set[n], params)
...
def get_feature_names(params, feat_set):
    ...
    for feat in all_feats:
        ...
        elif 'total_freq_bands' in params[feat_group].keys() and feat == 'spectral_edge_frequency':
...
def add_feat_data_to_array(all_data, new_data, feat_pd_names, feat_name, params):
    ...
    if feat_group == 'IBI':
        bands = np.array([0.5, 30])
    elif 'total_freq_bands' in params[feat_group].keys() and feat_name == 'spectral_edge_frequency':
        bands = np.array(params[feat_group]['total_freq_bands'])
    else:
        bands = np.array(params[feat_group]['freq_bands'])
    ...
    for b in range(N_bands):
        ...
        if a_feat_name not in feat_pd_names:
            raise ValueError('Feature name %s does not exist - check code' % a_feat_name)
